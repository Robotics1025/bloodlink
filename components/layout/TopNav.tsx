...
import { Bell, PanelLeft, Search, X, LogOut, User, Settings, ChevronDown } from "lucide-react";
      ...
      <header className="h-14 bg-white border-b border-slate-100 flex items-center justify-between px-4 shrink-0 gap-3">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <button onClick={() => setSearchOpen(true)}
            ...
            <span className="text-xs text-slate-400 truncate">Search requests, inventory, donors...</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {/* Avatar dropdown */}
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-2 border-l border-slate-100 pl-3 outline-none cursor-pointer rounded-lg pr-1 py-1 hover:bg-slate-100 transition-colors">
              <div className="hidden sm:block leading-none text-right">
                <p className="text-xs font-semibold text-slate-900 truncate max-w-[140px]">{name}</p>
